...
class AppLanguageEntryKeyModelAdmin(BaseModelAdmin):
    ...
    exclude = ["isPublic", "isActive"]
    formfield_overrides = {models.TextField: {"widget": AdminTextareaWidgetMine}}
    inlines = [AppLanguageTranslationEntryModelAdminInline]
    ...
    def flag_icon(self, obj):
        ...
        traducoes = " ".join(
            [
                f"<span><img width=48px src='{e.entry_language.language_icon.url}'></span>"
                for e in obj.translations_entries.all()
            ]
        )
